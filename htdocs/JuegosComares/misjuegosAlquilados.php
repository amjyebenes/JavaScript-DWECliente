...
try {
    ...
    if(isset($_POST['devolver'])){
        ControladorAlquiler::entregarJuego($_POST['devolver']);
        ControladorJuego::entregarJuego($_POST['devolver']);
    }
...
<html lang="en">
    ...
    <body class="d-flex justify-content-center">
        <div class="container row pt-3">
            ...
            <h2 class="text-center py-4">Mis juegos alquilados</h2>
            <?php
            if (isset($_SESSION['nombre'])) {
                $juegos = ControladorJuego::recuperarTodos();
                $alquileres = ControladorAlquiler::recuperarTodosAlquilados();
                if ($alquileres) {
                    ?>
                        ...
                        <table class="table table-striped
                            ...
                            <tbody class="table-group-divider">
                                ...
                                foreach ($alquileres as $a){
                                    if($a->DNI_cliente == $_SESSION['dni']){
                                        $juego = ControladorJuego::buscarJuego($a->Codigo_juego);
                                        if($juego->Alquilado == 'SI'){
                                            ...
                                            if (isset($_SESSION['nombre'])) {
                                                echo '<td><form action="" method="POST"><button class="btn btn-primary" name="devolver" value="' . $juego->Codigo . '">Devolver</button></form></td>';
                                            }
                                            echo '</tr>';
                                        }
                                    }
                    ...
                    <div class="alert alert-danger row justify-content-center gap-2">
                        ...
                        <span class="text-center fw-bold h4">No tienes ningun juego alquilado</span>
