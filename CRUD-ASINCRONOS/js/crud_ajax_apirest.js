...
la estructura básica de cualquier petición, encapsulando todo el código en la funcion ajax para luego llamarla*/
        ...
        const getAll = () => {
            ...
            ajax({
                url: `http://localhost:5555/${letrero}`,
                ...
                success: (respuesta) => {
                    ...
                    respuesta.forEach(el => {
                        ...
                        editar, el id, clase y profesor, para así poder después saber el contenido de los registros 
                        ...
                        let $clone = d.importNode($template, true); // CON TRUE VA LA INFORMACION DE DENTRO, CON FALSE VA SOLO LA ESTRUCTURA
...
//evento pra detectar envío de los datos de formulario 
            ...
            if (e.target === $form) {
                ...
                if (!e.target.id.value) {
                    ...
                    ajax({
                        ...
                        url: `http://localhost:5555/${letrero}/${e.target.id.value}`,
                        method: "PUT",
    ...
    en el formulario, para ello preguntamos si el origen coincide con la clase .delete*/
            if (e.target.matches(".delete")) {
                ...
                let isDelete = confirm(`¿Estás seguro de eliminar el ${e.target.dataset.id-1}?`);

                if (isDelete) {
                    //Delete - DELETE
                    ajax({
                        url: `http://localhost:5555/${letrero}/${e.target.dataset.id}`,
                        method: "DELETE",
                        success: (respuesta) => location.reload(),
                        error: (err) => alert(err)
                    });
